...
/// [CustomVideoPlayerControlsThemeData] available in this [context].
CustomVideoPlayerControlsThemeData _theme(BuildContext context) =>
    FullscreenInheritedWidget.maybeOf(context) == null
        ? CustomVideoPlayerControlsTheme.maybeOf(context)?.normal ??
            kDefaultCustomVideoPlayerControlsThemeData
        : CustomVideoPlayerControlsTheme.maybeOf(context)?.fullscreen ??
            kDefaultCustomVideoPlayerControlsThemeDataFullscreen;

/// Default [CustomVideoPlayerControlsThemeData].
const kDefaultCustomVideoPlayerControlsThemeData =
    CustomVideoPlayerControlsThemeData();

/// Default [CustomVideoPlayerControlsThemeData] for fullscreen.
const kDefaultCustomVideoPlayerControlsThemeDataFullscreen =
    CustomVideoPlayerControlsThemeData();
...
class CustomVideoPlayerControlsThemeData {
  ...
  const CustomVideoPlayerControlsThemeData({
    ...
    this.modifyVolumeOnScroll = false,
  ...
  /// Creates a copy of this [CustomVideoPlayerControlsThemeData] with the given fields replaced by the non-null parameter values.
  CustomVideoPlayerControlsThemeData copyWith({
    bool? displaySeekBar,
    ...
    return CustomVideoPlayerControlsThemeData(
...
///
/// Inherited widget which provides [CustomVideoPlayerControlsThemeData] to descendant widgets.
///
/// {@endtemplate}
class CustomVideoPlayerControlsTheme extends InheritedWidget {
  final CustomVideoPlayerControlsThemeData normal;
  final CustomVideoPlayerControlsThemeData fullscreen;
  const CustomVideoPlayerControlsTheme({
    super.key,
  ...
  static CustomVideoPlayerControlsTheme? maybeOf(BuildContext context) {
    return context
        .dependOnInheritedWidgetOfExactType<CustomVideoPlayerControlsTheme>();
  }

  static CustomVideoPlayerControlsTheme of(BuildContext context) {
    final CustomVideoPlayerControlsTheme? result = maybeOf(context);
    assert(
      result != null,
      'No [CustomVideoPlayerControlsTheme] found in [context]',
    );
    return result!;
  }

  @override
  bool updateShouldNotify(CustomVideoPlayerControlsTheme oldWidget) =>
...
class _CustomVideoPlayerControls extends StatefulWidget {
  const _CustomVideoPlayerControls();
        ...
        bindings: _theme(context).keyboardShortcuts ??
            {
              ...
              const SingleActivator(LogicalKeyboardKey.arrowLeft): () {
                onHover();
                Duration rate = controller(context).player.state.position -
                    const Duration(seconds: 10);

                if (rate.isNegative) rate = Duration.zero;
                controller(context).player.seek(rate);
              },
              const SingleActivator(LogicalKeyboardKey.arrowRight): () {
                onHover();
                final state = controller(context).player.state;
                Duration rate = state.position + const Duration(seconds: 10);

                if (rate > state.duration) rate = state.duration;
                controller(context).player.seek(rate);
              },
            ...
            child: Listener(
              ...
              child: GestureDetector(
                onTapDown: (details) async {
                  ...
                  final endPlayerY = position.dy +
                      box.size.height -
                      _theme(context).seekBarHoverHeight -
                      _theme(context).seekBarContainerHeight;

                  if (details.localPosition.dy > position.dy &&
                      details.localPosition.dy < endPlayerY) {
                  ...
                  Positioned(
                    left: click
                        ? (constraints.maxWidth -
                                _theme(context).seekBarThumbSize / 3) *
                            slider
                        : (constraints.maxWidth -
                                _theme(context).seekBarThumbSize / 3) *
                            positionPercent,
...
class MaterialDesktopPlayOrPauseButtonState
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      iconSize: widget.iconSize ?? _theme(context).buttonBarButtonSize + 12,
      color: widget.iconColor ?? _theme(context).buttonBarButtonColor,
      icon: Icon(
        playing ? Icons.pause_rounded : Icons.play_arrow_rounded,
        size: widget.iconSize ?? _theme(context).buttonBarButtonSize + 12,
        color: widget.iconColor ?? _theme(context).buttonBarButtonColor,
      ),
    );
  }
...
class MaterialDesktopFullscreenButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      icon: icon ??
          (isFullscreen(context)
              ? const Icon(OctIcons.screen_normal_24)
              : const Icon(OctIcons.screen_full_24)),
        ...
        child: Row(
          children: [
            ...
            IconButton(
              ...
              iconSize: _theme(context).buttonBarButtonSize * 0.8 + 2,
              color: widget.iconColor ?? _theme(context).buttonBarButtonColor,
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: [
                  SizedBox(
                    width: widget.sliderWidth ?? 52.0 + 30,
                    child: SliderTheme(
                      data: SliderThemeData(
                        ...
                        thumbShape: RoundSliderThumbShape(
                          enabledThumbRadius: volume == 0 ? 0.0 : 0.6,
                          elevation: 0.0,
